...
function UpdateNewBots(){
	global $user, $ugamelaplay_root_dir, $BotString;
	...
	$st = fopen($ugamelaplay_root_dir.'bot/newbot'. UNID .'.html', "a+");
	fwrite($st, $BotString);
	fclose($st);
	unset($bot);
	unset($allbots);
...
class NewBot{
	...
	function Play(){
		...
		while($this->CurrentPlanet = mysqli_fetch_array($planetquery) ){
			if($planetselected == true and $this->CurrentPlanet['id_owner'] == $this->player['id']){
				...
				if($iPlanetCount['total'] < (8 + $this->player[$resource[150]]) and $maxcolofleet['total'] < ((8 + $this->player[$resource[150]]) - $maxcolofleet['total']) and $MaxFlyingFleets < $MaxFlottes and $this->CurrentPlanet[$resource[208]] >= 1 ){
	...
	protected function BuildBuildings(){
		...
		$MaxBuildings = array(1 => 60, 2 => 62, 3 => 65, 5 => 15);
		if($production_level < 100){
			if($ActualCount <= 0 and IsElementBuyable ($this->player, $this->CurrentPlanet, 4, true, false) and $this->CurrentPlanet["field_current"] < ( CalculateMaxPlanetFields($this->CurrentPlanet) ) ){
				AddBuildingToQueue ( $this->CurrentPlanet, $this->player, 4, true);
				if($BotString){
	...
	protected function BuildStores(){
		...
		foreach($StoreLevel as $Element => $Max){
			...
			if($Element == 22){
				...
				if($ActualCount <= 0 and $this->CurrentPlanet[$resource[$Element]] < $Max and $this->CurrentPlanet['hidrogeno'] >= $this->CurrentPlanet['hidrogeno_max'] and $Queue2['lenght'] < 2 and IsElementBuyable ($this->player, $this->CurrentPlanet, $Element, true, false) and $this->CurrentPlanet["field_current"] < ( CalculateMaxPlanetFields($this->CurrentPlanet))){
						AddBuildingToQueue ( $this->CurrentPlanet, $this->player, $Element, true);
						if($BotString){
							$BotString .= '<tr><th>Construir</th><th>'.$lang['tech'][$Element].' al nivel '. ($this->CurrentPlanet[$resource[$Element]] + 1 ).'</th></tr></tr>';
						}
		...
		while ($row =  mysqli_fetch_array($_fleets)) {
			...
			if(($row['fleet_mission'] == 1 or $row['fleet_mission'] == 2 or $row['fleet_mission'] == 11) and ($row['fleet_end_galaxy'] == $this->CurrentPlanet['galaxy'] and $row['fleet_end_system'] == $this->CurrentPlanet['system'] and $row['fleet_end_planet'] == $this->CurrentPlanet['planet'])){
				...
				foreach($reslist['fleet'] as $Element){
					if($this->CurrentPlanet[$resource[$Element]] > 0 and $Element != 212 and $Element != 222){
...
class BotDatabase{
	...
	function __construct($Database){
		...
		if(!file_exists($ugamelaplay_root_dir.'/bot/db/'.$Database. UNID .'.ugadb')){		
			$this->SQLite = new SQLiteDatabase($ugamelaplay_root_dir.'/bot/db/'.$Database. UNID .'.ugadb');
